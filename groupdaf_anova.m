...
%patient1=allpatientgoodstuff.allpatientutterances(:,1)
...
%%patients
for ipat=1:16
pat_0{ipat,:} = find(allpatientgoodstuff.allpatientmsdelay(:,ipat) == 0)';
pat_0meanpause(ipat)= nanmean(allpatientgoodstuff.allpatientpauses(pat_0{ipat,:}))';
...
pat_0_utt{ipat,:} = find(allpatientgoodstuff.allpatientmsdelay(:,ipat) == 0)';
pat_0meanutterance(ipat)= nanmean(allpatientgoodstuff.allpatientutterances(pat_0_utt{ipat,:}))';
...
%% controls
for isubj=1:10
control_0{isubj,:} = find(allcontrolgoodstuff.allcontrolmsdelay(:,isubj) == 0)';
control_0meanpause(isubj)= nanmean(allcontrolgoodstuff.allcontrolpauses(control_0{isubj,:}))';
...
control_0_utt{isubj,:} = find(allcontrolgoodstuff.allcontrolmsdelay(:,isubj) == 0)';
control_0meanutterance(isubj)= nanmean(allcontrolgoodstuff.allcontrolutterances(control_0_utt{isubj,:}))';
...
daf_anova_data_utterance= [pat_50meanutterance pat_100meanutterance pat_200meanutterance pat_500meanutterance pat_1000meanutterance control_50meanutterance control_100meanutterance control_200meanutterance control_500meanutterance control_1000meanutterance];
group=[ones(1, 80) (ones(1, 50)*2)];
condition=[ones(1, pats) (ones(1, pats)*2) (ones(1, pats)*3) (ones(1, pats)*4) (ones(1, pats)*5) ones(1, subs) (ones(1, subs)*2) (ones(1, subs)*3) (ones(1, subs)*4) (ones(1, subs)*5)];
...
% pat1 pause
test=find(allpatientgoodstuff.allpatientmsdelay(:,1) == 200)
mean(allpatientgoodstuff.allpatientpauses(test,1))


ipat=1
cond=200
for ipat=1:16
patients_200_delay(ipat)=nanmean(allpatientgoodstuff.allpatientpauses((find(allpatientgoodstuff.allpatientmsdelay(:,ipat) == cond)),ipat))
end
...
nanmean(find(allpatientgoodstuff.allpatientmsdelay(:,1)== 200))
